...
class MegatronFake(object):
    def __init__(self) -> None:
        ...
        if model_scale == '1b':
            ...
            self.test_data = torch.load(f'./data/attention/cb-test-1b-fp32-depth{msa_depth}-{ckpt_iter}-test.pt') # [:-15]
            ...
            self.test_data = torch.load(f'./data/attention/cb-test-esm-fp32-depth{msa_depth}-{ckpt_iter}-test.pt') # [:-15]
...
def eval_unsupervised():
    ret_contect = []
    testset = np.load(f'corpus/cb513_test_dataset.npy', allow_pickle=True)
...
torch.save(ret_contect, f'./data/attention/ret/casp12-testset-{model_scale}-{msa_depth}-{ckpt_iter}.pt')
