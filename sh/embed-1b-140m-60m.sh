...
if [ $CKPT == 1b ]; then
       ...
       # MAX_ALIGNS=256
       MAX_LENGTH=1024
       ckpt_path=/dataset/ee84df8b/workspace/release/1024h-8l-8hd-1mbs-128gbs-1mp-32768tokens-256aligns-1024length-4000ws-200000iter-release
elif [ $CKPT == 60m ]; then
       LAYERNUM=6
       HIDDENSIZE=768
       HEAD=8
       ...
       # MAX_ALIGNS=512
       MAX_LENGTH=1024
       ckpt_path=/dataset/ee84df8b/workspace/release/768h-6l-8hd-1mbs-128gbs-1mp-32768tokens-512aligns-1024length-4000ws-200000iter-release
fi
